...
const StintLapsCompareGraph: React.FC<IStintCompareProps> = (props: IStintCompareProps) => {
  useEffect(() => {
  ...
  const graphDomain = { x: calcXDom(props.reference.laps), y: [_.min(lapTimeGaps), _.max(lapTimeGaps)] as DomainTuple };
  const tt = (
    <VictoryTooltip
...
export default StintLapsCompareGraph;
